...
document.getElementById("game").addEventListener("keydown", (ev) => {
  ...
  if (!window.timer && isLetter) {
    window.timer = setInterval(() => {
      ...
      const msPassed = curentTime - window.gameStart;
  ...
  if (isBackspace) {
    // Handling extra incorrect letters (spans)
    const extraIncorrect = currentWord.querySelectorAll('.letter.extra');
    if (extraIncorrect.length > 0) {
        const lastIncorrect = extraIncorrect[extraIncorrect.length - 1];
        lastIncorrect.remove();

        // Updating the cursor position after removing the extra incorrect letter
    ...
    if (currentLetter && isFirstLetter) {
      // Mov to the previous word and set the last letter
      removeClass(currentWord, 'current');
